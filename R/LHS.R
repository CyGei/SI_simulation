...
tails_pmf <- function(dens){
  ...
  if( is_positive[1] == FALSE){ #if the 1st value is 0
    ...
    dens <- replace(dens, starting_indices, val_to_replace ) # replace starting 0s with exp densities
  ...
  if( is_positive[length(is_positive)] == FALSE){ #if the last index returns 0
    ...
    dens <- replace(dens, ending_indices, val_to_replace ) # replace starting 0s with exp densities
